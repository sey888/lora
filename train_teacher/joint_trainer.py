...
class Joint_Trainer:
    def __init__(self, config_, save_path_):
        ...
        self.model = make(self.config['model'])
                # --- 插入验证代码开始 ---
        print("\n" + "="*40)
        print("正在验证分层 LoRA 秩配置...")
        print("="*40)
        
        # 针对 PanDA 模型结构，访问 core 属性下的 pretrained.blocks
        # self.model 是 PanDA 实例，self.model.core 是 DepthAnythingV2 实例
        for i, blk in enumerate(self.model.core.pretrained.blocks):
            if hasattr(blk.attn.qkv, 'linear_a_q'):
                # 读取实际注入的 LoRA 层的输出维度
                rank = blk.attn.qkv.linear_a_q.out_features
                print(f"层索引 [{i:02d}] | 状态: 已激活 | 当前 Rank: {rank}")
            else:
                print(f"层索引 [{i:02d}] | 状态: 未激活")
