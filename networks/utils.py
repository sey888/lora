...
class LoRA_Depth_Anything_v2(LoRA):
    ...
    def __init__(self, da_model: DepthAnythingV2, lora_ranks, lora_layer=None):
        super(LoRA_Depth_Anything_v2, self).__init__()

        assert all(r > 0 for r in lora_ranks.values())
        ...
        for t_layer_i, blk in enumerate(da_model.pretrained.blocks):
            ...
            current_rank = lora_ranks.get(t_layer_i, 0) # 获取当前层的秩，如果未指定则默认为0
            if current_rank == 0:
                continue # 如果秩为0，则跳过该层不应用LoRA
            w_a_linear_q = nn.Linear(self.dim, current_rank, bias=False)
            w_b_linear_q = nn.Linear(current_rank, self.dim, bias=False)
            w_a_linear_v = nn.Linear(self.dim, current_rank, bias=False)
            w_b_linear_v = nn.Linear(current_rank, self.dim, bias=False)
